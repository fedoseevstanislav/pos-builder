...
The locked frame defines only the `arch_blocks.tasks` branch below. Resume handoffs in this skill additionally use the top-level `pending_resume` string plus `pending_resume_tracker_slug` for the write-path pause.

```json
{
  "arch_blocks": {
    "tasks": {
      "status": "deferred_no_tracker | partial | needs_verification | done",
      ...
      "wow_moment_decomposition_parent_ref": "<tracker-native URL or identifier>",
      "rules_of_use_skipped": false,
      "my_architecture_skipped": false,
      "gaps": [
        "<string gap note>",
        {"slug": "<tracker_slug>", "reason": "<learner raw phrase>", "deferred_at_phase": "5"}
      ]
    }
  }
}
...
- Exactly one entry in `connected` has `is_agent_memory: true`.
...
- `backup.status` stays unset until Phase 10. After that it is `enabled` or `none_accepted` for every write-enabled tracker; `backup.enabled = false` alone does not satisfy G10.
- `rules_status` is unset until Phase 11. After that it is either `appended` or `deferred_read_only`.
